        ...
        if (
          ...
          if (
            ...
            if (订阅类型 === "mixed") {
              ...
              // 多国家代理IP: 优先URL参数proxyip，其次环境变量COUNTRY_PROXY (JP:ip,AU:ip,SG:ip)
              let 多国代理IP = url.searchParams.getAll("proxyip");
              if (多国代理IP.length === 0 && env.COUNTRY_PROXY) {
                多国代理IP = (await 整理成数组(env.COUNTRY_PROXY)).filter(
                  (item) => item.includes(":"),
                );
              }
              const 生成VLESS节点 = (原始地址列表, 前缀名, 代理路径覆盖) => {
                return 原始地址列表
                  .map((原始地址) => {
                    const regex =
                    ...
                    let 完整节点路径 = 代理路径覆盖 || config_JSON.完整节点路径;
                    if (前缀名) 节点备注 = 前缀名 + 节点备注;
                    ...
                    if (链式代理匹配) {
                      try {
                        ...
                        console.warn(
                          `[订阅内容] 链式代理解析失败: ${链式代理匹配[0]}`,
                        );
                      }
                    } else if (反代IP池.length > 0 && !代理路径覆盖) {
              ...
              if (多国代理IP.length > 0) {
                const 每批数量 = Math.max(
                  1,
                  Math.floor(完整优选IP.length / 多国代理IP.length),
                );
                const 原始路径 = config_JSON.完整节点路径;
                let 所有国家节点 = [];
                for (let i = 0; i < 多国代理IP.length; i++) {
                  const 条目 = 多国代理IP[i];
                  const 含国家代码 =
                    条目.includes(":") && /^[A-Z]{2}/.test(条目);
                  const 国家代码 = 含国家代码 ? 条目.split(":")[0] : "";
                  const 代理地址 = 含国家代码
                    ? 条目.slice(条目.indexOf(":") + 1)
                    : 条目;
                  const 名称前缀 = 国家代码 ? `[${国家代码}]` : "";
                  const 当前代理路径 = 代理地址
                    ? `${原始路径}/proxyip=${代理地址}`.replace(/\/\//g, "/")
                    : null;
                  const 本批IP = 完整优选IP.slice(
                    i * 每批数量,
                    (i + 1) * 每批数量,
                  );
                  const 本批节点 = 生成VLESS节点(
                    本批IP,
                    名称前缀,
                    当前代理路径,
                  );
                ...
                订阅内容 = 其他节点LINK + 所有国家节点.join("\n");
              } else {
                订阅内容 =
                  其他节点LINK + 生成VLESS节点(完整优选IP, "", null).join("\n");
              }
            } else {
              // 订阅转换
              const 代理IP参数 = url.searchParams
                .getAll("proxyip")
                .map((ip) => `&proxyip=${encodeURIComponent(ip)}`)
                .join("");
              const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(url.protocol + "//" + url.host + "/sub?target=mixed&token=" + 今日订阅转换后端专属TOKEN + "&asOrg=" + 识别运营商(request) + (url.searchParams.has("sub") && url.searchParams.get("sub") != "" ? `&sub=${url.searchParams.get("sub")}` : "") + 代理IP参数)}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=${config_JSON.订阅转换配置.SUBEMOJI}&scv=${config_JSON.跳过证书验证}`;
              try {
